...
class PriorBox(object):
    ...
    def __init__(self, cfg, min_size, max_size, batch=1):
    ...
    def forward(self):
        ...
        for k, f in enumerate(self.feature_maps):
            ...
            # rel size: min_size
            ar = 1.5
            s_k_i = self.min_sizes[k] / self.image_size[1] / sqrt(ar)
            s_k_j = self.min_sizes[k] / self.image_size[0] * sqrt(ar)
        ...
        if self.batch > 1:
            output = torch.stack([output for _ in range(self.batch)], dim=0)
        return output
